...
class Arrow {
  ...
  void draw(rive.Renderer renderer) {
    ...
    renderer.save();
    var arrowTransform = rive.Mat2D.fromTranslation(translation).mul(
        rive.Mat2D.fromRotation(rive.Mat2D(), atan2(heading.y, heading.x)));
...
class CentaurGame extends rive.RenderTexturePainter {
  ...
  static const int maxApples = 5000;
  ...
  bool paint(rive.RenderTexture texture, Size size, double elapsedSeconds) {
    ...
    if (batchRender) {
      rive.Rive.batchAdvanceAndRender(
          _apples.map((apple) => apple.machine), elapsedSeconds, renderer);
    } else {
      rive.Rive.batchAdvance(
          _apples.map((apple) => apple.machine), elapsedSeconds);
    }
